...
class AutoCommit {
  ...
  detectChangeType(filePath, content) {
    ...
    if (fileName.includes('README') || fileName.includes('DOCS') || fileExt === '.md') {
      return 'docs';
    } else if (fileName.includes('test') || fileName.includes('spec')) {
      return 'test';
    } else if (fileName.includes('config') || fileName.includes('package.json')) {
      return 'chore';
  ...
  generateDescription(content, fileName) {
    ...
    if (contentLower.includes('ecosystem') && contentLower.includes('integration')) {
      return 'Ecosystem integration and coordination features';
    } else if (contentLower.includes('mcp') && contentLower.includes('service')) {
      return 'MCP service integration for Cursor';
    } else if (contentLower.includes('northflank') && contentLower.includes('deploy')) {
      return 'NorthFlank deployment configuration';
    } else if (contentLower.includes('websocket') && contentLower.includes('communication')) {
      return 'Real-time WebSocket communication';
    } else if (contentLower.includes('monitoring') && contentLower.includes('dashboard')) {
      return 'Real-time monitoring dashboard';
    } else if (contentLower.includes('ai') && contentLower.includes('coordination')) {
      return 'AI agent coordination across ecosystem';
    } else if (contentLower.includes('knowledge') && contentLower.includes('base')) {
      return 'Hybrid Knowledge Base functionality';
    } else if (contentLower.includes('rules') && contentLower.includes('enforcement')) {
      return 'Ecosystem rules and enforcement';
    } else if (contentLower.includes('documentation') && contentLower.includes('storage')) {
      return 'Documentation storage in Knowledge Base';
    } else if (contentLower.includes('analytics') && contentLower.includes('ecosystem')) {
      return 'Ecosystem analytics and monitoring';
    } else if (contentLower.includes('error') && contentLower.includes('prevention')) {
      return 'Error prevention and troubleshooting';
    } else if (contentLower.includes('secrets') && contentLower.includes('management')) {
      return 'Centralized secrets management';
    } else if (contentLower.includes('format') || contentLower.includes('cleanup')) {
      return 'Code formatting and cleanup';
    } else if (contentLower.includes('update') && contentLower.includes('dependencies')) {
    ...
    } else if (contentLower.includes('add') && contentLower.includes('feature')) {
      return 'New feature implementation';
    } else if (contentLower.includes('refactor') && contentLower.includes('code')) {
  ...
  async commitChanges(filePath, content) {
    try {
      ...
      const commitMessage = this.generateCommitMessage(filePath, 'auto', content);
  ...
  generateBulkCommitMessage(changedFiles) {
    ...
    const projects = projectList.length === 1 ? projectList[0] : 'Multiple projects';
